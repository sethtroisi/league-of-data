...
def _dnn_model_fn(features, labels, mode, params, config=None):
    ...
    with variable_scope.variable_scope(
            ...
            values=tuple(features.values())) as input_layer_scope:
        if all([
...
class MyClassifier(estimator.Estimator):
  ...
  * for each `column` in `feature_columns`:
  ...
  def predict_classes(self, input_fn=None, batch_size=None,
    ...
    preds = super(MyClassifier, self).predict(
        input_fn=input_fn,
...
class MyRegressor(estimator.Estimator):
  ...
  * for each `column` in `feature_columns`:
  ...
  def evaluate(self,
    ...
    return super(MyRegressor, self).evaluate(
        input_fn=input_fn,
        feed_fn=feed_fn,
        batch_size=batch_size,
        steps=steps,
  ...
  def predict(self, input_fn=None, batch_size=None, outputs=None,
              as_iterable=True):
    ...
    if not outputs:
      return self.predict_scores(
          ...
          batch_size=batch_size,
          as_iterable=as_iterable)
    return super(MyRegressor, self).predict(
        input_fn=input_fn,
        batch_size=batch_size,
        outputs=outputs,
        as_iterable=as_iterable)
  ...
  def predict_scores(self, x=None, input_fn=None, batch_size=None,
    ...
    preds = super(MyRegressor, self).predict(
        x=x,
        input_fn=input_fn,
        batch_size=batch_size,
        outputs=[key],
  ...
  def export(self,
    ...
    return super(MyRegressor, self).export(
...
class MyEstimator(estimator.Estimator):
  ...
  estimator = DNNEstimator(
      ...
      hidden_units=[1024, 512, 256])
  If your label is keyed with "y" in your labels dict and you want to enable
  centered bias,
